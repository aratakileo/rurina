...
Usage:
  import clipboard
  clipboard.copy('The text to be copied to the clipboard.')
...
def init_dev_clipboard_clipboard():
    def copy_dev_clipboard(text):
        ...
        if text == '':
            warnings.warn(
                'clipboard cannot copy a blank string to the clipboard on Cygwin. This is effectively a no-op.')
        if '\r' in text:
...
def init_no_clipboard():
    class ClipboardUnavailable(object):
        ...
        def __call__(self, *args, **kwargs):
            raise ClipboardException(EXCEPT_MSG)
...
def determine_clipboard():
    ...
    if 'cygwin' in platform.system().lower():  # Cygwin has a variety of values returned by platform.system(), such as 'CYGWIN_NT-6.1'
        if os.path.exists('/dev/clipboard'):
            warnings.warn(
                'clipboard\'s support for Cygwin is not perfect, see https://github.com/asweigart/pyperclip/issues/55')
...
def lazy_load_stub_copy(text):
    ...
    This allows users to import clipboard without having determine_clipboard()
...
def waitForPaste(timeout=None):
    ...
    This function raises clipboardTimeoutException if timeout was set to
    ...
    while True:
        ...
        if timeout is not None and time.time() > startTime + timeout:
            raise ClipboardTimeoutException('waitForPaste() timed out after ' + str(timeout) + ' seconds.')
...
def waitForNewPaste(timeout=None):
    ...
    while True:
        ...
        if timeout is not None and time.time() > startTime + timeout:
            raise ClipboardTimeoutException('waitForNewPaste() timed out after ' + str(timeout) + ' seconds.')
